...
def plot_GAMP_v_others(p, n_list):
  output_list = load('output_list (sparse, noiseless).npy')
  ...
  plt.errorbar(delta_list, mean_corr1_list_AM, yerr=SD_corr1_list_AM, marker='x', color='red', ecolor='red', elinewidth=1, capsize=7, label="AM (lasso)")
  ...
  plt.savefig('GAMP_v_others_sparse_sigma0.1(beta1).pdf', bbox_inches='tight')
  plt.show()
  ...
  plt.errorbar(delta_list, mean_corr2_list_AM, yerr=SD_corr2_list_AM, marker='x', color='red', ecolor='red', elinewidth=1, capsize=7, label="AM (lasso)")
  ...
  plt.xticks(fontsize=16)
  plt.yticks(fontsize=16)
  plt.savefig('GAMP_v_others_sparse_sigma0.1(beta2).pdf', bbox_inches='tight')
  plt.show()
